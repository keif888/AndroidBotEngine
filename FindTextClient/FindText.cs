...
namespace FindTextClient
{
    ...
    public class FindText
    {
        ...
        private static int PicFindInternal(int mode, uint colour, uint n, FindDirection findDirection, IntPtr bitmap, int Stride, int zw, int zh, int sx, int sy, int sw, int sh, sbyte[] ss, uint[] s1, uint[] s0, string text, int w, int h, int err1, int err0, GDIFunctions.POINT[] allpos, int allpos_max)
        {
            int ok = 0;
            int v = 0;
            int len1 = 0; int len0 = 0;
            byte[] gs;
            int o;
            int i;
            int j;
            ...
            if (mode == (int)FindMode.multiColourMode)
            {
                max = (int)n;
                v = (int)(colour * colour);
                for (i = 0, k = 0, colour = (uint)0, o = 0; (j = (int)text[o++]) != 0;)
                {
                    if ((j >= 48) && (j <= 57))
                        colour = (uint)(colour * 10 + (j - 48));
                    if ((j == 47) || (text[o] == 0))
                    {
                        if (k == 0)
                            s1[i] = (uint)((colour >> 16) * Stride + (colour & 0xFFFF) * 4);
                        else
                            s0[i++] = (uint)colour;
                        colour = (uint)0;
                    }
            ...
            else
            {
                ...
                for (y = 0; y < h; y++)
                {
                    for (x = 0; x < w; x++)
                    {
                        i = (int)((mode == (int)FindMode.colourPositionMode) ? y * Stride + x * 4 : y * sw + x);
                        if (text[o++] == 49)
                            s1[len1++] = (uint)i;
                        else
                            s0[len0++] = (uint)i;
                ...
                if (err0 >= len0)
                    len0 = 0;
                max = (int)((len1 > len0) ? len1 : len0);
                //----------------------
                // Color Position Mode
                // only used to recognize multicolored Verification Code
                //----------------------
                //if (mode == (int)FindMode.colourPositionMode)
                //    goto StartLookUp;
                //else
                if (mode != (int)FindMode.colourPositionMode)
                {
                    ...
                    //----------------------
                    o = (int)(sy * Stride + sx * 4);
                    j = (int)(Stride - sw * 4);
                    i = 0;
                    if (mode == (int)FindMode.colourMode)  // Color Mode
                    {
                        rr = (int)((colour >> 16) & 0xFF);
                        gg = (int)((colour >> 8) & 0xFF);
                        bb = (int)(colour & 0xFF);
                        for (y = 0; y < sh; y++, o += j)
                        {
                            for (x = 0; x < sw; x++, o += 4, i++)
                            {
                                r = (int)(Marshal.ReadByte(bitmap, 2 + o) - rr);
                                g = (int)(Marshal.ReadByte(bitmap, 1 + o) - gg);
                                b = (int)(Marshal.ReadByte(bitmap, o) - bb);
                                v = (int)(r + rr + rr);
                                ss[i] = (sbyte)((((1024 + v) * r * r + 2048 * g * g + (1534 - v) * b * b) <= n) ? 1 : 0);
                            }
                        }
                    }
                    else if (mode == (int)FindMode.greyThresholdMode)  // Gray Threshold Mode
                    {
                        colour = (colour + 1) << 7;
                        for (y = 0; y < sh; y++, o += j)
                        {
                            for (x = 0; x < sw; x++, o += 4, i++)
                            {
                                ss[i] = (sbyte)((((Marshal.ReadByte(bitmap, 2 + o) * 38) + (Marshal.ReadByte(bitmap, 1 + o) * 75) + (Marshal.ReadByte(bitmap, o) * 15)) < colour) ? 1 : 0);
                            }
                        }
                    }
                    else if (mode == (int)FindMode.greyDifferenceMode)  // Gray Difference Mode
                    {
                        gs = new byte[sw * sh];
                        ss.CopyTo(gs, sw * sh);  // gs=(unsigned char *)(ss+sw*sh);
                        x2 = (int)(sx + sw);
                        y2 = (int)(sy + sh);
                        for (y = (int)(sy - 1); y <= y2; y++)
                        {
                            for (x = (int)(sx - 1); x <= x2; x++, i++)
                            {
                                ...
                                {
                                    o = (int)(y * Stride + x * 4);
                                    gs[i] = (byte)(((Marshal.ReadByte(bitmap, 2 + o) * 38) + (Marshal.ReadByte(bitmap, 1 + o) * 75) + (Marshal.ReadByte(bitmap, o) * 15)) >> 7);
                                }
                            }
                        }
                        ...
                        for (y = (int)1; y <= sh; y++)
                        {
                            for (x = (int)1; x <= sw; x++, i++)
                            {
                                o = (int)(y * k + x);
                                n = (uint)(gs[o] + colour);
                                ss[i] = (sbyte)(((gs[o - 1] > n) || (gs[o + 1] > n) || (gs[o - k] > n) || (gs[o + k] > n) || (gs[o - k - 1] > n) || (gs[o - k + 1] > n) || (gs[o + k - 1] > n) || (gs[o + k + 1] > n)) ? 1 : 0);
                            }
                        }
                    }
                    else
                    {
                        r = (int)((colour >> 16) & 0xFF);
                        g = (int)((colour >> 8) & 0xFF);
                        b = (int)(colour & 0xFF);
                        rr = (int)((n >> 16) & 0xFF);
                        gg = (int)((n >> 8) & 0xFF);
                        ...
                        for (y = 0; y < sh; y++, o += j)
                        {
                            for (x = 0; x < sw; x++, o += 4, i++)
                            {
                                r = (int)Marshal.ReadByte(bitmap, 2 + o);
                                g = (int)Marshal.ReadByte(bitmap, 1 + o);
                                b = (int)Marshal.ReadByte(bitmap, o);
                                ss[i] = (sbyte)(((r >= r_min) && (r <= r_max) && (g >= g_min) && (g <= g_max) && (b >= b_min) && (b <= b_max)) ? 1 : 0);
                            }
                        }
                    }
                }
            ...
            int y1;
            //StartLookUp:
            if ((mode == (int)FindMode.multiColourMode) || (mode == (int)FindMode.colourPositionMode))
            {
                x1 = (int)sx;
                y1 = (int)sy;
                x2 = (int)(sx + sw - w);
            ...
            // if ((direction < 1) || (direction > 8))  // Using an enum, so this can't happen.
            //    direction = (int)1;
            // 1 ==> Top to Bottom ( Left to Right )
            // 2 ==> Top to Bottom ( Right to Left )
            // 3 ==> Bottom to Top ( Left to Right )
            ...
            // 7 ==> Right to Left ( Top to Bottom )
            // 8 ==> Right to Left ( Bottom to Top )
            int direction = (int)findDirection;
            if ((--direction) > 3)
            {
                i = (int)y1;
                y1 = (int)x1;
            ...
            bool noMatch = false;
            int y3;
            for (y3 = (int)y1; y3 <= y2; y3++)
            {
                int x3;
                for (x3 = (int)x1; x3 <= x2; x3++)
                {
                    y = (int)(((direction & 3) > 1) ? y1 + y2 - y3 : y3);
                    x = (int)((direction & 1) == 1 ? x1 + x2 - x3 : x3);  // x=(dir&1) ? x1+x2-x3 : x3;
                    if (direction > 3)
                    {
                        i = (int)y;
                        y = (int)x;
                    ...
                    int e1 = (int)err1;
                    int e0 = (int)err0;
                    if (mode == (int)FindMode.multiColourMode) // (5)
                    {
                        o = (int)(y * Stride + x * 4);
                        for (i = 0; i < max; i++)
                        {
                            j = (int)(o + s1[i]);
                            colour = (uint)s0[i];
                            r = (int)(Marshal.ReadByte(bitmap, 2 + j) - ((colour >> 16) & 0xFF));
                            g = (int)(Marshal.ReadByte(bitmap, 1 + j) - ((colour >> 8) & 0xFF));
                            b = (int)(Marshal.ReadByte(bitmap, j) - (colour & 0xFF));
                    ...
                    else if (mode == (int)FindMode.colourPositionMode) //  (3)
                    {
                        o = (int)(y * Stride + x * 4);
                        j = (int)(o + colour);
                        rr = (int)Marshal.ReadByte(bitmap, 2 + j);
                        gg = (int)Marshal.ReadByte(bitmap, 1 + j);
                        bb = (int)Marshal.ReadByte(bitmap, j);
                        for (i = 0; i < max; i++)
                        {
                            if (i < len1)
                            {
                                j = (int)(o + s1[i]);
                                r = (int)(Marshal.ReadByte(bitmap, 2 + j) - rr);
                                g = (int)(Marshal.ReadByte(bitmap, 1 + j) - gg);
                                b = (int)(Marshal.ReadByte(bitmap, j) - bb);
                                v = (int)(r + rr + rr);
